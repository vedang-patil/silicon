...
std::vector<Move> generatePseudoLegalMoves(const Board& board)
{
    ...
    else
    {
        ...
        canCastleKingside = canCastleKingside && (((attacksTo(board, 60) | attacksTo(board, 61)) & board.getOccupancyBitboard(!board.currentState.colour)) != 0);
        canCastleQueenside = canCastleQueenside && (((attacksTo(board, 60) | attacksTo(board, 59)) & board.getOccupancyBitboard(!board.currentState.colour)) != 0);
